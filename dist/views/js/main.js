...
var resizePizzas = function(size) {
  window.performance.mark("mark_start_resize");   // User Timing API function (the time at start resize)
  ...
  window.performance.mark("mark_end_resize"); // the time of end resize
  window.performance.measure("measure_pizza_resize", "mark_start_resize", "mark_end_resize"); //measure the time between start and end resize
...
function updatePositions() {
  ...
  // replace querySelectorAll by getElementsByClassName for better perf
  var items = document.getElementsByClassName('.mover');
  var scrollTop = (document.body.scrollTop)/1250;
  var phase = [];
  // only need 5 values for phases for every scroll
  for (var i =0; i < 5; i++){
    phase.push(Math.sin(scrollTop + i)*100);
  }
  for (var i = 0; i < items.length; i++) {
    items[i].style.transform ='translateX('+ items[i].basicLeft + phase[i%5] + 'px'+ ')';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // reduce the number of pizzas to better perf
  for (var i = 0; i < 16; i++) {
